...
try: 
    while (active):
       ...
       # print("Pressure, hPa= ", sensor.pressure)
        ...
        data = {"pressure: ": movingAverageGeneral(lastHumid)}
        response = authed_session.post(db+path, json=data)
        ...
        path = "CO2 Moving Average.json"

        data = {"pressure: ": movingAverageGeneral(lastCO2)}
        response = authed_session.post(db+path, json=data)
        
        path = "TVOC Moving Average.json"

        data = {"pressure: ": movingAverageGeneral(lastTVOC)}
        response = authed_session.post(db+path, json=data)
